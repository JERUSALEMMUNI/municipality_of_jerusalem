...
class DropdownSearchSelectBox(DropdownSearch):
    ...
    def clear_selected_items(self):
        ...
        Then we click on the select box twice if it is not clicked
        if it is clicked, we only click it once
        """
        self.click_button()
        ...
        log.info(f"selected options list in {self.label}: {self.list}")

    def validate_checked_list_count(self):
        """
        click the dropdown menu
        will creat a list from the list elements under the fiels (div)
        will compare the already made self.list earlier with the one we created
        the function returns tuple values of table of unequal values, and the table itself
        """
        self.click_button()
        ...
        for i in range(0, len(list)):
            if self.list[i] != list[i]:
                unequal_values.append(["no. " + str(i), self.list[i], list[i]])
        return self.list == list, unequal_values
    ...
    def select_element(self, pre):
        ...
        try to select option if exist
        if not, the option value will be none
        then will check if the option is checked or not
        if its checked it will append a value to the list, else it will remove
        """
        self.click_button()
        try:
