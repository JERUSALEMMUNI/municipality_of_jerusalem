...
class ObjectionPropertyTaxes(BasePage):
    ...
    def create_page_widgets(self):
        ...
        self.widgets['פרטי הנכס וסיבת ההשגה_שם פרטי'] = create_widget('TextField', style=self.style, label='שם פרטי', index=2)

        self.widgets['פרטי מגיש הבקשה_שם משפחה'] = create_widget('TextField', style=self.style, label='שם משפחה')
        self.widgets['פרטי הנכס וסיבת ההשגה_שם משפחה'] = create_widget('TextField', style=self.style, label='שם משפחה', index=2)

        self.widgets['פרטי מגיש הבקשה_סוג זיהוי'] = create_widget('DropdownSearch', style=self.style, label='סוג זיהוי')
        self.widgets['פרטי הנכס וסיבת ההשגה_סוג זיהוי'] = create_widget('DropdownSearch', style=self.style, label='סוג זיהוי', index=2)

        self.widgets['פרטי מגיש הבקשה_מספר ת.ז.'] = create_widget('TextField', style=self.style, label='מספר ת.ז.')
        self.widgets['פרטי הנכס וסיבת ההשגה_מספר ת.ז.'] = create_widget('TextField', style=self.style, label='מספר ת.ז.', index=2)

        self.widgets['פרטי מגיש הבקשה_מספר דרכון'] = create_widget('TextField', style=self.style, label='מספר דרכון')
        self.widgets['פרטי הנכס וסיבת ההשגה_מספר דרכון'] = create_widget('TextField', style=self.style, label='מספר דרכון', index=2)

        self.widgets['פרטי מגיש הבקשה_מספר ח.פ.'] = create_widget('TextField', style=self.style, label='מספר ח.פ.')
        self.widgets['פרטי הנכס וסיבת ההשגה_.מספר ח.פ'] = create_widget('TextField', style=self.style, label='מספר ח.פ.', index=2)

        self.widgets['פרטי מגיש הבקשה_מספר עמותה'] = create_widget('TextField', style=self.style, label='מספר עמותה')
        self.widgets['פרטי הנכס וסיבת ההשגה_מספר עמותה'] = create_widget('TextField', style=self.style, label='מספר עמותה', index=2)

        self.widgets['פרטי מגיש הבקשה_מספר מוסד'] = create_widget('TextField', style=self.style, label='מספר מוסד')
        self.widgets['פרטי הנכס וסיבת ההשגה_מספר מוסד'] = create_widget('TextField', style=self.style, label='מספר מוסד', index=2)

        self.widgets['פרטי מגיש הבקשה_טלפון נייד'] = create_widget('PhoneField', style=self.style, label='טלפון נייד')
        self.widgets['פרטי הנכס וסיבת ההשגה_טלפון נייד'] = create_widget('PhoneField', style=self.style, label='טלפון נייד', index=2)

        self.widgets['פרטי מגיש הבקשה_דוא"ל'] = create_widget('TextField', style=self.style, label='דוא"ל')
        self.widgets['פרטי הנכס וסיבת ההשגה_דוא"ל'] = create_widget('TextField', style=self.style, label='דוא"ל', index=2)

        self.widgets['פרטי מגיש הבקשה_ממלא ההשגה'] = create_widget('DropdownSearch', style=self.style, label='ממלא ההשגה')

        self.widgets['פרטי מגיש הבקשה_טלפון קווי'] = create_widget('PhoneField', style=self.style, label='טלפון קווי')
        self.widgets['פרטי הנכס וסיבת ההשגה_טלפון קווי'] = create_widget('PhoneField', style=self.style, label='טלפון קווי', index=2)
