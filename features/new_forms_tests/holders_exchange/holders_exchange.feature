...
Feature: HoldersExchange form - scenarios step 1
  ...
  Scenario: TC_HOLDERSEXCH_01 - registration of a new tenant by the owner of the property
    ...
    When from parent "פרטי מגיש" write a valid value "דני" in "שם פרטי"
    When from parent "פרטי מגיש" write a valid value "גלעד" in "שם משפחה"
    When from parent "פרטי מגיש" pick "ת.ז." from "סוג זיהוי"
    When from parent "פרטי מגיש" write a valid value "332796184" in "מספר ת.ז."
    When from parent "פרטי מגיש" fill "058-8078687" as valid value in "טלפון נייד"
    When from parent "פרטי מגיש" fill "02-8078687" as valid value in "טלפון קווי"
    When from parent "פרטי מגיש" write a valid value "@TEMP_EMAIL_ADDRESS"" in "דוא"ל"
...
#    Then 5th Validate if went back to expected form
    ...
    When click on "המשך" button
    When from parent "פרטי מחזיק חדש" write a valid value ""דני"" in "שם פרטי"
