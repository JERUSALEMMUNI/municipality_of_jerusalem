Feature: DirectContactToAssessment form - scenarios step 1

  - Form name: פנייה ישירה לאגף שומה וגבייה
  - Feature file name: Direct_Contact_To_Assessment_step_1.feature
  - Form link: https://jeronlineforms.jerusalem.muni.il/DirectContactToAssessment
  - Number of Pages is : 3, We are at step: 1
  - All fields are mandatory except (טלפון קווי)

  Scenario: CONTACTASSESSM_01 הזנת ערכים תקינים בשדות "פרטי מגיש הבקשה"
    Given Navigate to "DirectContactToAssessment" form
...
#    When clear fields
  ...
  Scenario: CONTACTASSESSM_05 הזנת ערכים שגוים בשדות "פרטי מגיש הבקשה"
    ...
    When write an invalid value "000000000" in "מספר ת.ז."
    Then validate "מספר ת.ז." error is "מספר זהות לא תקין"
