...
#    When click on "המשך"
  ...
  Scenario: EXCEPTVOLUNT_09 הזנת ערכים שגויים ב"פרטי מוסד"
    ...
    Then check if "מספר רישום" error is "תווים X יש להזין מספר מעל/עד"
    When write an invalid value "ssسسדד" in "מספר רישום"
    Then check if "מספר רישום" error is "יש להזין ספרות בלבד"
    ...
    Then check if "המטרות העיקריות של המוסד" error is "שפת קלט לא תקינה"
    ...
    Then check if "פירוט מלא של התמורה שהמוסד גובה עבור שירותיו/פעולותיו" error is "שפת קלט לא תקינה"

    When write an invalid value "ee" in textarea of "הקריטריונים/תנאי הזכאות לקבלת שירותים מהמוסד"
    Then check if "הקריטריונים/תנאי הזכאות לקבלת שירותים מהמוסד" error is "שפת קלט לא תקינה"

    When write an invalid value "ee" in textarea of "פירוט האוכלוסיה הנהנית משירותי המוסד"
    Then check if "פירוט האוכלוסיה הנהנית משירותי המוסד" error is "שפת קלט לא תקינה"

    When write an invalid value "ee" in "האם המוסד קשור לארגון/תאגיד מקומי או ארצי אחר"
    Then check if "האם המוסד קשור לארגון/תאגיד מקומי או ארצי אחר" error is "שפת קלט לא תקינה"

    When write an invalid number "ee" in "אחוז תושבי ירושלים המקבלים שירות מהעמותה מבין כלל מקבלי השירותים מהעמותה"
    Then check if "אחוז תושבי ירושלים המקבלים שירות מהעמותה מבין כלל מקבלי השירותים מהעמותה" error is "יש להזין ספרות בלבד"
    When write an invalid number "111" in "אחוז תושבי ירושלים המקבלים שירות מהעמותה מבין כלל מקבלי השירותים מהעמותה"
    Then check if "אחוז תושבי ירושלים המקבלים שירות מהעמותה מבין כלל מקבלי השירותים מהעמותה" error is "שדה לא תקין"