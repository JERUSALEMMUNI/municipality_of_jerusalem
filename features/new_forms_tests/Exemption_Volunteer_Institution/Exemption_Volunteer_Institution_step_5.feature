...
Feature: ExemptionVolunteerInstitution form - scenarios step 5
  ...
  Scenario: EXCEPTVOLUNT_12 טעינת קבצים שגויים ב"מסמכים"
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "תעודת רישום כתאגיד" files list
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "מסמכי יסוד של המוסד (תקנון, פרוטוקול ישיבת ועד אחרונה)" files list
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "דו"ח כספי מבוקר ע"י רואה חשבון לשנה שלפני האחרונה*" files list
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "דו"ח כספי לשנה החולפת*" files list
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "תקציב המוסד לשנה הנוכחית" files list
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "אישור בתוקף על ניהול פנקסי חשבונות ורשומות ממס הכנסה (אישור ניכוי מס ואישור על ניהול ספרים בתוקף)" files list
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "אישור בדבר ניהול תקין תקף" files list
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "אישור מלכ"ר ממשרדי מע"מ" files list
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "אישור מוסד ציבורי" files list
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "דו"ח המפרט את עלויות השכר של חמשת מקבלי השכר הגבוה ביותר, כולל ציון תפקידם במוסד, חתום ע"י עורך דין" files list
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "חוזה שכירות או נסח טאבו" files list
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "תצהיר מבעל תפקיד בכיר בעמותה, חתום על ידי עו"ד, כי דירות המגורים עומדות בתנאים שנקבעו בחוזר המנכ"ל משרד הפנים 01/2012 לגבי מתן פטור ביחס לדירות מגורים" files list
    ...
    Then validate "10MB_file_to_upload.pdf" file is not in "הצהרה חתומה של ממלא הבקשה חתומה ומאושרת ע"י עו"ד" files list
