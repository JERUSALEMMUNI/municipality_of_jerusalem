...
#todo: make a way to make pycharm know the different between  דוא"ל and דוא''ל
  Scenario:
    ...
    When click on "המשך" button
    When from table "פרטי המיקום ומועדי הצילומים" add "2"
    When from table "פרטי המיקום ומועדי הצילומים" remove "2"
    ...
    When from table "פרטי המיקום ומועדי הצילומים" at row "2" write an invalid value "w" in "שם האתר"
    Then from table "פרטי המיקום ומועדי הצילומים" at row "2" check if "שם האתר" error is "יש להזין אותיות בעברית בלבד ותווים מיוחדים " / () ' . , _ -"
